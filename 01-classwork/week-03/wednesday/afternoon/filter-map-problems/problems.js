...
function convertirTemps(array) {
  const celsiusToFahrenheit = (tempFarenheit) => {
    const celsius = (tempFarenheit - 32) * (5 / 9);
    return celsius;
  };
  return array.map(celsiusToFahrenheit);
}
...
function cuentaCaracter(array) {
  const NletraStrings = (string) => {
    return string.length;
  };
  return array.map(NletraStrings);
}
...
function contieneFalso(array) {
  ...
  return array.filter(falsevalue);
}
...
function palabrasDeLongitud(array, minLongitud) {
  minLongitud = 4;
  return array.filter((item) => item.length > minLongitud);
}
